function converted_nifti = DICOMConvert(img_Path)
...
converted_nifti = nifti_Images.files{1};
if (length(nifti_Images.files) == 2)
    ...
    if File1.bytes > File2.bytes
        ...
        converted_nifti = char(nifti_Images.files{1});
    elseif File2.bytes > File1.bytes
        delete(nifti_Images.files{1});
        converted_nifti = char(nifti_Images.files{2});
    end
else
    converted_nifti = char(nifti_Images.files{1});
end
